  ...
  return (
    <AppLayout title="My Redemptions">
      <div className="mb-8">
        ...
        <p className="text-muted-foreground">Track all your reward redemptions.</p>
      </div>
      {isLoading ? (
          ...
          {redemptions.map(r => (
            <div key={r.id} className="bg-card rounded-2xl border border-border p-5 flex items-center gap-4">
              ...
              <div className="flex-1 min-w-0">
                <div className="font-semibold text-foreground">Redemption #{r.id}</div>
                <div className="text-sm text-muted-foreground">{r.pointsSpent.toLocaleString()} points spent</div>
                {r.confirmationCode && <div className="text-xs font-mono text-primary mt-0.5">Code: {r.confirmationCode}</div>}
              </div>
              <div className="flex items-center gap-2 flex-shrink-0">
                {STATUS_ICONS[r.status]}
                <Badge variant="outline" className={cn("text-xs capitalize", r.status === "confirmed" ? "border-emerald-200 text-emerald-700" : r.status === "cancelled" ? "border-red-200 text-red-700" : "border-amber-200 text-amber-700")}>
                  {r.status}
                </Badge>
              </div>
            </div>
